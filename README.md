...
- **管理员**：新建/编辑/删除数据库连接、项目、AI 供应商；勾选同步对象、启停同步、立即同步、重置游标；起草和保存存储过程转换、清理变更日志。
...
| 策略 | 触发条件 | 能力 |
|---|---|---|
| `TIMESTAMP` | 存在 `update_time` / `updated_at` / `last_modified` 等**时间戳类型**列 | 检测新增 **和** 更新 |
| `IDENTITY` | 存在创建时间列，或单列数字主键 | **仅**检测新增 |
